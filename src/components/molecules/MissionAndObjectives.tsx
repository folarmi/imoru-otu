...
const MissionAndObjectives = () => {
  return (
    ...
    <div>
      ...
      <section className="my-20 flex flex-col sm:flex-row flex-wrap items-stretch justify-center gap-6 sm:gap-x-12 px-4">
        {missionAndVision.map(({ desc, icon: Icon, id, name }) => {
          return (
            <div
              className="w-full sm:max-w-[552px] drop-shadow-card rounded-xl p-6 flex flex-col gap-y-6 border border-secondary/20"
              key={id}
            >
              <Icon className="w-10 h-10 sm:w-12 sm:h-12 text-secondary" />
              <p className="text-secondary font-semibold text-2xl sm:text-3xl playfair">
                {name}
              </p>
              <p className="text-green_200 text-base sm:text-lg">{desc}</p>
            </div>
          );
        })}
      </section>
      ...
      {/* Objectives Section */}
      <section className="bg-yellow_200 py-20 px-4">
        <HeaderText
          title="Our Objectives"
          subtitle="Core goals that guide our work and define our impact"
        />

        <div className="mt-16 space-y-6 w-full sm:w-3/4 mx-auto">
          {Array.from(
            { length: Math.ceil(Objectives.length / 3) },
            (_, rowIndex) => (
              <div
                ...
                {Objectives.slice(rowIndex * 3, rowIndex * 3 + 3).map(
                  ({ id, name }) => {
                    return (
                      <div
                        key={id}
                        className="grid grid-cols-[auto_1fr] gap-4 items-start bg-white p-5 sm:p-6 rounded-lg w-full border border-gray-200"
                      >
                        <div className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-secondary/10 flex-shrink-0">
                          <p className="font-secondary font-bold text-sm sm:text-base text-secondary">
                            {id}
                          </p>
                        </div>
